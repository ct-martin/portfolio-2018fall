<template>
  <div>
    <h2 class="title">Pains of Stateless State Management</h2>
 ...
 redisClient.exists(`gameOf.${sessionid}`, (err, inGame) => {
  ...
  return cb2(-1);
    ...
    <p>
      ...
      Additionally, since I need an error variable for each query, I’ve ended up appending the level of nesting to the end of it, which is bad practice and then I end up with <code>err9</code> and such for variable names.
    </p>
    ...
    <p>
      Finally, I haven’t finished debugging why yet, but I’ve found that non-<code>HttpOnly</code> cookies seem to not get passed to the client when behind Cloudflare’s proxy.
      I know this isn’t a proper way to identify users, but it works for the purposes of what I’m trying to accomplish and I need the <code>sessionid</code> accessible to the JavaScript to identify the client to the server.
    ...
    <p>
      ...
      For example, the OOP <code>this.game.me.ships</code> (<code>this.game.player(this).ships</code> if static) would be <code>game(sessionid).me().ships()</code>.
      While this isn’t “proper” OOP per say, it works, and I had some bad luck when I tried using ES6 class or Node’s <code>module.exports</code> to do OOP methods before.
